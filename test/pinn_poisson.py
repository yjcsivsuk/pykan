...
class PhysicsInformedNN():
    ...
    def net_f(self, x, t):
        ...
        f = u_xx + u_tt + torch.pi **2 *(1+4*t**2)* torch.sin(torch.pi * x) * torch.sin(torch.pi * t**2)-2*torch.pi*torch.sin(torch.pi*x)*torch.cos(torch.pi*t**2)
    ...
    def plot(self):
        ...
        plt.savefig('./test/poisson_pinn.png')
        plt.show()
...
if __name__ == '__main__':
    ...
    u_up = np.zeros((N_u // 4, 1), dtype=float)
    u_down = np.zeros((N_u // 4, 1), dtype=float)
